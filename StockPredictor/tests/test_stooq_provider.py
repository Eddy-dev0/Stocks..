...
def test_stooq_tolerates_noise_lines_in_payload() -> None:
    """Non-CSV noise lines should be ignored instead of failing tokenization."""

    async def _runner() -> None:
        provider = StooqProvider()
        csv_text = """# generated by upstream edge node
Date,Open,High,Low,Close,Volume
2024-01-02,1,1.5,0.9,1.2,100
429,too,many,requests
2024-01-03,2,2.5,1.8,2.1,150
"""
        url_log: list[str] = []
        provider._client = _DummyClient(csv_text, url_log)
